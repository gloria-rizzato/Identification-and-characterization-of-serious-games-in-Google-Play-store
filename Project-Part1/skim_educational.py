def if_serious_education(app_id):
    ...
    word_learn = ['screening', 'dyslexia', 'autism', 'diagnosis', 'adhd', 'health', 'dysgraphia','dyscalculia',
                  'knowledge', 'teach', 'skill', 'train', 'skills', 'thinking']
    word_child = ' '.join(map(str, word_child))
    word_game = ' '.join(map(str, word_game))
    word_learn = ' '.join(map(str, word_learn))
    json_result_list = json_result.split(" ")
    words_list_child = word_child.split(" ")
    word_list_game = word_game.split(" ")
    words_list_learn = word_learn.split(" ")
    ...
    if len(common_word_game) != 0:
        ...
        if len(common_word_child) != 0:
            common_word_learn = list(set(json_result_list) & set(words_list_learn))
            if len(common_word_learn) != 0:
